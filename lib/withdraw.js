...
module['exports'] = function (options, callback) {
  ...
  wallet.get(options.id, function(err, result){
    ...
    result.save(function(err, result){
      result.transaction = { ip: options.ip || "0.0.0.0", 'amount': options.amount.toFixed(), 'currency': options.currency, 'owner': result.owner, id: result.id, address: options.address, price: options.price };
      callback(err, result);
    });
  });
};